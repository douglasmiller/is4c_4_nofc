...
//--------------------------------------------------------------------
	...
	$query_eqq = "select * from dlog where department = 45 AND tdate > '$EOS' and register_no = $laneno order by tdate";
	...
	if ($num_rows_eqq > 0) {
		...
		$query_eqq = "select SUM(total) from dlog where tdate > '$EOS' and register_no = $laneno AND department = 45";
		$result_eqq = mysql_query($query_eqq, $db_a);
		$row_eqq = mysql_fetch_array($result_eqq);
