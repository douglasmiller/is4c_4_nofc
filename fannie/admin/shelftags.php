...
$testQ = "SELECT if(u.brand IS NULL,'',substring(u.brand,1,12)) as brand,  
		if(u.pack_size IS NULL,'',u.pack_size) as size,  
		...
		if(LENGTH(order_no) != 5, 'Misc', 'UNFI') as vendor, 
		ROUND(p.normal_price,2) AS normal_price,
		p.scale AS scale
	FROM " . PRODUCTS_TBL . " as p left outer join product_details as u  on p.upc = u.upc  
