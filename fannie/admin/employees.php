<?php include("/pos/fannie/src/passwd.php"); ?>
<?php
ob_start();
$page_title = 'Fannie - Admin Module';
$header = 'Employee Management';
include_once('../src/header.php');
include_once('../src/functions.php');
// echo '<link rel="stylesheet" href="'.SRCROOT.'/tablesort.css" type="text/css" />';

if (isset($_POST['submit']) || isset($_POST['add'])) {
	foreach ($_POST AS $key => $value) {
		$$key = $value;
	}
}

if (isset($_POST['submit'])) {
	foreach ($id AS $emp_no) {
		if ($JobTitle[$emp_no] == 'MANAGER') $sec = 20; else $sec = 10;
		if ($EmpActive[$emp_no] == 'on') $active = 1; else $active = 0;
		$updateQ = "UPDATE employees SET AdminPassword = '" . escape_data($pass[$emp_no]) . "', FirstName = '" . escape_data($FirstName[$emp_no]) . "', LastName = '" . escape_data($LastName[$emp_no]) . "', EmpActive=$active, JobTitle = '{$JobTitle[$emp_no]}', frontendsecurity = $sec, backendsecurity = $sec WHERE emp_no = $emp_no";
		$updateR = mysql_query($updateQ);
		if (!$updateR) {echo '<p><font color="red">One or more employees could not be updated. Please try again.</font> Query: ' . $updateQ . '</p>';}
	}
	$max = $_POST['add'];
	if ($EmpActive[$max] == 'on' && $_POST['submit'] == 'Add') {
		if ($JobTitle[$max] == 'MANAGER') $sec = 20; else $sec = 10;
		$insertQ = "INSERT INTO employees VALUES ($max, $max, " . escape_data($pass[$max]) . ", '" . escape_data($FirstName[$max]) . "', '" . escape_data($LastName[$max]) . "', '{$JobTitle[$max]}', 1, $sec, $sec, '', '')";
		$insertR = mysql_query($insertQ) or DIE('<p><font color="red">The new employee could not be added. Please try again.</font></p>' . mysql_error() . '<br />Query: ' . $insertQ);
		// if (mysql_affected_rows() != 1) {
		// 	echo '<p><font color="red">The new employee could not be added. Please try again.</font></p>';
		// }
	}
}

if (isset($_POST['pdf'])) {
	ob_end_clean();
	define('FPDF_FONTPATH','../src/fpdf/font/');
	require('../src/fpdf/fpdf.php');
	$order = $_POST['sort'];
	$query = "SELECT emp_no, FirstName
		FROM employees
		WHERE EmpActive=1
		ORDER BY $order ASC";
	$result = mysql_query($query);
	
	$pdf=new FPDF('P', 'mm', 'Letter');
	$pdf->SetMargins(5, 14);
	$y = 14;
	$pdf->SetAutoPageBreak('off', 0);
	//$pdf->AddPage('P');
	$pdf->SetFont('Courier', 'B', 14);
	
	$yMax = 250;
	$xMax = 175;
	
	// 215.9 x 279.4 mm
	
	for ($i = 1; $i <= 2; $i++) {
		$pdf->AddPage('P');
		$y = 14;
		
		$height = 6;
		$width = 40;
		$query = "SELECT emp_no, FirstName
			FROM employees
			WHERE EmpActive=1
			ORDER BY $order ASC";
		$result = mysql_query($query);
		$cell = 1;
		$num = mysql_num_rows($result);
		$box_h = (ROUND($num / 2, 0) + 1) * $height;
		$box_w = ($width * 4) + 5;
		
		// Draw some boxes in case it's not an even number of employees.
		if ($i == 2 && !isset($once)) {
			$once = TRUE;
			$y = $y + 10;
			$pdf->Rect(5, $y, $box_w, $box_h);
		} elseif ($i == 1 && !isset($box)) {
			$box = TRUE;
			$pdf->Rect(5, $y, $box_w, $box_h);
		}
		
		$pdf->SetXY(5, $y);
		$pdf->Cell($width, $height, 'Cashier No.', 1, 'C');
		$pdf->SetXY(5 + $width, $y);
		$pdf->Cell($width, $height, 'First Name', 1, 'C');
		$pdf->SetXY(5 + ($width * 2), $y);
		$pdf->Cell(5, $height, '', 1);
		$pdf->SetXY(10 + ($width * 2), $y);
		$pdf->Cell($width, $height, 'Cashier No.', 1, 'C');
		$pdf->SetXY(10 + ($width * 3), $y);
		$pdf->Cell($width, $height, 'First Name', 1, 'C');
		$y = $y + $height;
		
		while ($row = mysql_fetch_array($result)) {
			// 2 Columns.
			
			if ($cell % 2 == 1) { // First Column
				$pdf->SetXY(5, $y);
				$pdf->Cell($width, $height, $row[0], 1, 'C');
				$pdf->SetXY(5 + $width, $y);
				$pdf->Cell($width, $height, $row[1], 1, 'C');
				$pdf->SetXY(5 + ($width * 2), $y);
				$pdf->Cell(5, $height, '', 1);
			} elseif ($cell % 2 == 0) { // Second Column
				$pdf->SetXY(10 + ($width * 2), $y);
				$pdf->Cell($width, $height, $row[0], 1, 'C');
				$pdf->SetXY(10 + ($width * 3), $y);
				$pdf->Cell($width, $height, $row[1], 1, 'C');
				$y = $y + 6; // New Row
			}
			
			
			$cell++;
		}
	}
	$pdf->Output();
	
	exit();
}

$query = "SELECT * FROM employees ORDER BY emp_no DESC";
$result = mysql_query($query);

$maxQ = "SELECT MAX(emp_no) FROM employees";
$maxR = mysql_query($maxQ);
$max = mysql_result($maxR, 0) + 1;

// echo '<form action="employees.php" method="POST">
// 	<h3 align="center">Select Sort Order</h3>
// 	<p align="center">First Name<input type="radio" name="sort" value="FirstName">
// 	Cashier Number<input type="radio" name="sort" value="emp_no" checked="checked">
// 	<p align="center"><button name="pdf" type="submit">Generate Printable List</button></p>
// 	</form>';
echo '<form action="employees.php" method="POST">';
echo "<table border=0 cellspacing=0 cellpadding=5 align=center>";
echo "<thead><tr>\n
	<th class='sortable-numeric'>Emp. No.</th>\n
	<th>Pass</th>\n
	<th class='sortable-text'>Last Name</th>\n
	<th class='sortable-text'>First Name</th>\n
	<th filter-type='ddl' class='sotrable-text'>Job Title</th>\n
	<th>Mem #</th>\n<th>Wage</th>
	<th filter-type='ddl' class='sortable-numeric'>Active?</th>&nbsp;\n</tr></thead>";
$bg = '#eeeeee';
$max;
echo "<tbody>";
echo "<tr bgcolor='$bg'>";
echo "<td>".$max."</td>";
echo "<td><input type='password' name='pass[" . $max . "]' maxlength='6' size='6'></td>";
echo '<td><input type="text" name="LastName[' . $max . ']" maxlength="20" size="15"></td>';
echo '<td><input type="text" name="FirstName[' . $max . ']" maxlength="20" size="15"></td>';
echo "<td><select name='JobTitle[$max]'>
      <option value='STAFF'>Staff</option>
      <option value='MANAGER'>Manager</option>
      </select></td>";
echo "<td><input type='text' name='card_no[" . $max . "]' size='6'></td>";
echo "<td><input type='text' name='pay_rate[" . $max . "]' size='6'></td>";
echo "<td><input type='checkbox' name='EmpActive[" . $max . "]' CHECKED>";
echo "</td>";
echo "<td><input type='hidden' name='add' value='" . $max . "'>&nbsp;</td>
	<td><input type='submit' name='submit' value='Add'></td></tr>\n";

while ($row = mysql_fetch_row($result)) {
	$bg = ($bg=='#eeeeee' ? '#ffffff' : '#eeeeee'); // Switch the background color.
	$id = $row[0];
	echo "<tr bgcolor='$bg'>";
	echo "<td>".$row[0]."</td>";
	echo "<td><input type='password' name='pass[" . $id . "]' maxlength=6 size=6 value='" . $row[2] . "'></td>";
	echo '<td><input type="text" name="LastName[' . $id . ']" maxlength="20" size="15" value="' . $row[4] . '"></td>';
	echo '<td><input type="text" name="FirstName[' . $id . ']" maxlength="20" size="15" value="' . $row[3] . '"></td>';
	echo "<td><select name='JobTitle[$id]'>
		<option value='STAFF'";
	if ($row[5] == 'STAFF') echo ' SELECTED';
	echo ">Staff</option>
		<option value='MANAGER'";
	if ($row[5] == 'MANAGER') echo ' SELECTED';
	echo ">Manager</option>
		</select></td>";
	echo "<td><input type='text' name='card_no[". $id ."]' value='" . $row[9] . "' size=6></td>";
	echo "<td><input type='text' name='pay_rate[" . $id . "]' value='" . $row[10] . "' size=6></td>";
	echo "<td><input type='checkbox' name='EmpActive[" . $id . "]'";
	if ($row[6] == 1) echo ' CHECKED';
	echo "></td>";
	echo "<td><input type=hidden name='id[]' value=".$row[0].">&nbsp;</td></tr>\n";
}

echo "<tr><td><input type='submit' name='submit' value='submit'></td></tr>";
echo "</tbody></table></form>";
echo "<div style='text-align:right; font-size:0.75em;'><a href='" . $_SERVER["PHP_SELF"] . "?logout=1'>Logout</a></div>";

include ('../src/footer.php');
ob_end_flush();
?>

<script>
function filterTable(pstrValue) {
    if( pstrValue == 'none' ){
        $('#output > tbody > tr').show();
    }
    else {
        $('#output > tbody > tr').hide();
        $('#output > tbody > tr:contains('+pstrValue+')').show();
    }
}

$(document).ready(function() {
	var options = {
		clearFiltersControls: [$('#clearfilters')]
	};
	$('#output').tableFilter();
});


</script>
