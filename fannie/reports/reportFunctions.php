...
function hash_total($table,$date1,$date2) {
	...
	$hashQ = "SELECT ROUND(sum(total),2) AS HASH_sales
		...
		WHERE date(datetime) >= '$date1'
		AND date(datetime) <= '$date2'
		AND department IN (40,41,42,44,46,47,48)
		AND trans_subtype NOT IN ('IC', 'MC', 'CP')
		AND trans_status <> 'X'
		AND emp_no <> 9999";
...
//echo $query;
	...
	echo "<p><b>" . $title . " Subtotal: $" . $tot . " (". number_format($pct,2) ."%)</b></p>\n";
	// select_to_table($query,0,$bgcolor);
}

function list_dept_totals($gross,$table,$date1,$date2,$bgcolor) {
	if (!isset($date2)) $date2 = $date1;
	$query = "SELECT * FROM " . DB_NAME . ".depts_dev WHERE group_no <> 0";
	$result = mysql_query($query);
	echo "<table border=0>";
	while ($row0 = mysql_fetch_assoc($result)) {
		// $depts = explode($row0['dept_name'],",");
		$in = $row0['dept_name'];
		...
		$query1 = "SELECT ROUND(sum(d.total),2) AS total,ROUND((SUM(d.total)/$gross)*100,2) as pct
			...
			AND d.department IN($in)
			AND d.trans_status <> 'X' AND d.emp_no <> 9999";
		$results1 = mysql_query($query1);
		$row1 = mysql_fetch_row($results1);
		$tot = $row1[0];
		$pct = $row1[1];

		echo "<tr style='font-size:1em;font-weight:bolder;'><td colspan=3>" . $row0['group_name'] . "</td></tr><tr><td colspan=3>";
		
		$inventoryDeptQ = "SELECT t.dept_name,ROUND(sum(d.total),2) AS total,ROUND((SUM(d.total)/$gross)*100,2) as pct
		   	FROM (SELECT dept_no, dept_name FROM " . DB_NAME . ".departments WHERE dept_no IN($in)) t 
			LEFT JOIN  " . DB_LOGNAME . ".$table AS d ON t.dept_no = d.department
			AND date(d.datetime) BETWEEN '$date1' AND '$date2'
			AND d.department IN (" . $in . ")
			AND d.trans_subtype NOT IN('IC','MC','CP')
			AND d.trans_status <> 'X' AND d.emp_no <> 9999
			GROUP BY t.dept_no ORDER BY t.dept_no";
		// echo $inventoryDeptQ;
		select_to_tableNH($inventoryDeptQ,0,$bgcolor);	
		echo "</td></tr>";
		echo "<tr><td align='right'><i>" . ucwords(strtolower($row0['group_name'])) . " Total</i></td><td align='right'><b>$" . money_format('%n',$tot) . "</b></td><td align='right'>" . number_format($pct,2) . "%</td>";
	}
	echo "<tr style='font-weight:bolder; font-size:1em;'><td align='right'>Total</td><td align='right'>$" . $gross . "</td><td>&nbsp;</td></tr>";
	echo "</table>";
}
...
function tax_total($table,$date1,$date2) {
	$taxQ = "SELECT -SUM(total)
		FROM " . DB_LOGNAME . ".$table
		WHERE DATE(datetime) BETWEEN '$date1' AND '$date2'
		AND trans_type = 'A'
		AND emp_no <> 9999 AND trans_status <> 'X'";
	$taxR = mysql_query($taxQ);
	list($tax_total) = mysql_fetch_row($taxR);
	$tax_total = (is_null($tax_total)) ? 0 : $tax_total;
...
function net_total($table, $date1, $date2) {
	...
	$net = $gross + $hash + $tax_total + $totalDisc /*- $SSDdiscount2*/ + $pt_total + $coupons + $strchg + $RA;
	//
